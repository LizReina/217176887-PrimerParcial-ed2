...
//      que retorne cual seria el predecesor inorden de dicho dato, sin realizar el recorrido en inOrden.      
   ...
   public K predecesorInOrden(K clave){
      return predecesorInOrdenp(this.raiz, clave);
   }   
      
     private K predecesorInOrdenp(NodoMVias<K,V> nodoActual,K claveABuscar) {
       int pos=0;
           ...
           if(!existeClaveEnElNodo(nodoActual, claveABuscar)){
               ...
               return null;
                   ...
                   if(!nodoActual.esHijoVacio(pos)){
                    ...
                    Stack<NodoMVias<K,V>> pilaNueva=pilaDeNodos;
                  while(!pilaNueva.isEmpty()){
                    nodoActual=pilaNueva.pop();
                   for (int i = 0; i < nodoActual.cantidadDeDatosNoVacios(); i++) {
                    if(nodoActual.getClave(i).compareTo(claveABuscar)<0){
                        int posDato=porDondeBajar(nodoActual, claveABuscar)-1;
                       return nodoActual.getClave(posDato);
                    }
                   }
   ...
   */
   //Para un árbol m-vias implemente un método que retorne la cantidad de nodos que tienen
    ...
    public int cantidadNodosDistintoVacio(NodoMVias<K,V> nodoActual,int nivelABuscar,int nivel){
     ...
     public int cantidadNodosPadres(int nivel){
        return cantidadNodosPadres(this.raiz,nivel,0);
    }
    
     public int cantidadNodosPadres(NodoMVias<K,V> nodoActual,int nivelABuscar,int nivel){
       if(NodoMVias.esNodoVacio(nodoActual)){
           return 0;
       }
       ...
       int cantidad=0;
        ...
        for (int i = 0; i < orden; i++) {
            cantidad=cantidad+cantidadNodosPadres(nodoActual.getHijo(i),nivelABuscar,nivel+1);
        }
