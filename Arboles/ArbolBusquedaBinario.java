...
public class ArbolBusquedaBinario <K extends Comparable<K>,V> 
  ...
  public  void meterHijosPorIzquierda(NodoBinario<K,V> nodoActual,Stack<NodoBinario<K,V>> pilaDeNodos ){
         while(!NodoBinario.esNodoVacio(nodoActual)){
 ...
 public K sucesorInOrden(K clave)throws ExceptionClaveNoExiste{
     ...
     return buscarSucesorInOrden(nodoActual);
    }
           
    private K buscarSucesorInOrden(NodoBinario<K,V> nodoABuscar)throws ExceptionClaveNoExiste{
     NodoBinario<K,V> nodoActual=this.raiz;
     if(NodoBinario.esNodoVacio(nodoActual)){
         throw new ExceptionClaveNoExiste("Arbol vacio");
     }
            ...
            if(nodoActual.getClave().compareTo(nodoABuscar.getClave())== 0){
                if(!nodoActual.esVacioHijoDerecho()){
                    ...
                    return nodoActual.getClave();
                     ...
                     Stack<NodoBinario<K,V>> pilaCopia=new Stack<>();
                     pilaCopia=pilaDeNodos;
                       while(!pilaCopia.isEmpty()){
                         NodoBinario<K,V>  nodoCopia=pilaCopia.pop();
                         if(nodoCopia.getClave().compareTo(nodoABuscar.getClave())>0){
                             return nodoCopia.getClave();
                         }
                        return null;
                       }
                }
     ...
     public K predecesorInOrden(K clave)throws ExceptionClaveNoExiste{
      NodoBinario<K,V> nodoActual=new NodoBinario<>();
      nodoActual.setClave(clave);
     return buscarPredecesorInOrden(nodoActual);
    }
           
    private K buscarPredecesorInOrden(NodoBinario<K,V> nodoABuscar)throws ExceptionClaveNoExiste{
     NodoBinario<K,V> nodoActual=this.raiz;
     if(NodoBinario.esNodoVacio(nodoActual)){
         throw new ExceptionClaveNoExiste("Arbol vacio");
     }
     ...
     Stack<NodoBinario<K,V>> pilaDeNodos=new Stack<>();
        meterHijosPorDerecha(nodoActual, pilaDeNodos);
 ...
 ///  Implementar un método que retorne verdadero si un árbol binario es un montículo, falso en caso contrario.
 //     En un montículo el dato que almacena un nodo cualquiera siempre es menor que los datos de sus descendientes
    ...
    public boolean esMonticulo(){
      ...
      Queue<NodoBinario<K,V>> colaDeodos=new LinkedList<>();
      colaDeodos.offer(this.raiz);
       boolean monticulo=true;
      while(!colaDeodos.isEmpty() && monticulo==true){
          NodoBinario<K,V> nodoActual=colaDeodos.poll();
          
           if(!nodoActual.esVacioHijoDerecho()){
               K clave=nodoActual.getHijoDerecho().getClave();
           ...
           if(!nodoActual.esVacioHijoIzquierdo()){
               K clave=nodoActual.getHijoIzquierdo().getClave();
               if(nodoActual.getClave().compareTo(clave)>0){
                   monticulo=false;
               }
                nodoActual=nodoActual.getHijoIzquierdo();
